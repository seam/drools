...
{
   private static final long serialVersionUID = -1958250222223530L;
   ...
   @Inject @Generic KnowledgeBase kbase;
      
   @Inject @Scanned @Generic KnowledgeBase scannedKbase;
   
   @Produces @Default @SessionScoped
   public StatefulKnowledgeSession produceStatefulSession() throws Exception
   {
      StatefulKnowledgeSession ksession = kbase.newStatefulKnowledgeSession(
            configUtils.getKnowledgeSessionConfiguration(), null);
      
      if (!config.getDisableSeamDelegate())
      {
   ...
   {
      StatefulKnowledgeSession ksession = scannedKbase.newStatefulKnowledgeSession(
            configUtils.getKnowledgeSessionConfiguration(), null);
   ...
   {
      StatelessKnowledgeSession ksession = scannedKbase.newStatelessKnowledgeSession(
            configUtils.getKnowledgeSessionConfiguration());
